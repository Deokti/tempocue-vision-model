...
def load_dataset(dataset: Path) -> tuple[torch.Tensor, torch.Tensor, list[list[tuple]]]:
    """Кадры (N,3,320,320), цели (N,1,80,80) и списки центров для метрик."""
    records = [
        json.loads(line)
        for line in (dataset / "labels.jsonl").read_text(encoding="utf-8").splitlines()
    ]
    frames = np.empty((len(records), 3, CANONICAL_SIDE, CANONICAL_SIDE), dtype=np.float32)
...
def evaluate(
    ...
    with torch.no_grad():
        for start in range(0, len(fold), batch_size):
            batch = fold.frames[start : start + batch_size].to(device)
...
def save_predictions(
    ...
    with torch.no_grad():
        found = decode_heatmap(model(fold.frames[:count].to(device)).cpu())

    side = CANONICAL_SIDE
    sheet = Image.new("RGB", (side * count + 8 * (count - 1), side), (24, 24, 24))
    for index in range(count):
        rgb = (fold.frames[index].numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
...
def run_epoch(
    ...
    for start in range(0, len(shuffled), batch_size):
        ...
        batch = fold.frames[batch_index].to(device)
        target = fold.targets[batch_index].to(device)
        optimizer.zero_grad()
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss = run_epoch(model, train, optimizer, device, args.batch_size)
